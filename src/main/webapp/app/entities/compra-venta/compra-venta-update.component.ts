...
export class CompraVentaUpdateComponent implements OnInit {
  ...
  /* la variable editform es el formulario (Todos sus campos)
     Todos los cambios a realizar se deben realizar sobre el objeto editform!!
   */
  ...
  updateForm(compraVenta: ICompraVenta) {
    /* Nuevo evento sobre el campo select de vehiculo:
     * Si cambia el vehiculo -> cambiar el precio
     */
    this.editForm.get('vehiculo').valueChanges.subscribe((event) => {
      this.editForm.get('precioTotal').setValue(event.precio*1.21)
    })
    this.editForm.get('precioTotal').disable();
    //Si es una creacion y no una modificacion -> campo fecha desabilitado
    if(compraVenta.id==undefined){
      this.editForm.get('fechaVenta').disable();
    }
    this.editForm.patchValue({
      id: compraVenta.id,
      /* Angular para las fechas usa moment()
         Para crear la fecha actual -> moment(new Date()) -> crea objeto fecha
       */
      fechaVenta: compraVenta.fechaVenta!=undefined ? compraVenta.fechaVenta : moment(new Date()),
